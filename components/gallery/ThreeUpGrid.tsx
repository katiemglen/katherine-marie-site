      ...
      {images.map((src, i) => (
        ...
        >
          <TiltCard className="cursor-pointer overflow-hidden rounded-xl will-animate relative group h-full">
            <ProgressiveImage src={wpImage(src, IMG_SIZES.gallery)} alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" onClick={() => onImageClick(i)} />
          </TiltCard>
        </motion.div>
      ))}
    </motion.div>
